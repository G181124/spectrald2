...
def main():
    ...
    parser.add_argument("--username", help="Username yang ingin dicari", required=True, nargs='+')
    ...
    # Multithreading jika --fast diaktifkan
    if args.fast:
        with ThreadPoolExecutor() as executor:
            futures = [executor.submit(run_username_check, username, verbose=args.verbose) for username in args.username]
            for future in futures:
                future.result()  # Menunggu semua thread selesai
    else:
        for username in args.username:
            run_username_check(username, verbose=args.verbose)
